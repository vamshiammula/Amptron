...
|                  |                                                          |
| ---------------- | -------------------------------------------------------- |
| **Legal name**   | Amptron Manufacturing Pvt. Ltd.                          |
| **Brand name**   | Amptron                                                  |
| **Tagline**      | Powering India's Electric Future                         |
| **HQ**           | Plot 42, Sector 8, IMT Manesar, Gurugram, Haryana 122051 |
| **Web**          | [www.amptron.co.in](https://www.amptron.co.in)           |
...
| Asset                   | Rule                                        | Example                                      |
| ----------------------- | ------------------------------------------- | -------------------------------------------- |
| Company                 | Amptron on all public surfaces              | Amptron                                      |
| Legal line              | Full entity in footers, contracts, invoices | Amptron Manufacturing Pvt. Ltd.              |
| Product                 | `Amptron` + one-word model, Title Case      | Amptron Volt, Amptron Storm, Amptron Cruise  |
| Model slug              | lowercase, hyphenated                       | `amptron-volt`                               |
| Signature CTA (riders)  | Verb + Amptron or the action                | Buy Amptron, Book a Test Ride                |
| Signature CTA (dealers) | Verb + Amptron                              | Stock Amptron                                |
| Portal                  | Function + Amptron, not a cute product name | Amptron Dealer Portal, Amptron Admin Console |
| Hashtags                | Brand + path                                | `#Amptron` `#BuyAmptron` `#StockAmptron`     |
...
| Use                                     | File                               | Wordmark colour        | Slash                |
| --------------------------------------- | ---------------------------------- | ---------------------- | -------------------- |
| Light backgrounds (nav, print on white) | `src/assets/images/logo.svg`       | Amptron Navy `#0A1F44` | Logo Green `#35D192` |
| Dark backgrounds (footer, navy hero)    | `src/assets/images/logo-light.svg` | White `#FFFFFF`        | Logo Green `#35D192` |
| App / favicon                           | `public/favicon.svg`               | Navy (icon drawing)    | Green slash          |
...
- Do not put the navy wordmark on navy, or the white wordmark on white or fog.
...
| ---------------- | ----------------- | --------- | ------------- | ------------------------------------------------------------ |
| **Amptron Navy** | `--navy`          | `#0A1F44` | 10, 31, 68    | Primary brand, body text, dark wordmark, theme-color         |
| **Navy Deep**    | `--navy-deep`     | `#061430` | 6, 20, 48     | Footer, deepest panels, text on teal buttons                 |
| **Signal Teal**  | `--teal`          | `#00C896` | 0, 200, 150   | CTAs, eyebrows, links, icon strokes, focus rings, stats band |
| **Soft Teal**    | `--teal-soft`     | `#E6FAF5` | 230, 250, 245 | Icon wells, selected chips, tinted success surfaces          |
| **White**        | `--white`         | `#FFFFFF` | 255, 255, 255 | Default page, light wordmark                                 |
| **Fog**          | `--fog`           | `#F5F7FA` | 245, 247, 250 | Cards, quote wells, alt sections                             |
| **Line / Mist**  | `--line` `--mist` | `#E2E8F0` | 226, 232, 240 | Borders; secondary text on navy                              |
| **Slate**        | `--slate`         | `#64748B` | 100, 116, 139 | Secondary body text on light                                 |

**Logo-only green** (not a UI fill):

| Name                          | Hex       | Role                         |
| ----------------------------- | --------- | ---------------------------- |
| **Logo Green**                | `#35D192` | Wordmark slash only          |
| Favicon slash (close variant) | `#3FD598` | A-mark in `favicon.svg` only |
...
| Name         | Token            | Hex       | Use                     |
| ------------ | ---------------- | --------- | ----------------------- |
| Danger       | `--danger`       | `#FF8080` | Errors on dark          |
| Danger deep  | `--danger-deep`  | `#B42318` | Errors on light         |
| Warning      | `--warning`      | `#B45309` | Warning text            |
| Warning soft | `--warning-soft` | `#FEF3C7` | Warning wells           |
| Info         | `--info`         | `#0369A1` | Informational text      |
| Info soft    | `--info-soft`    | `#E0F2FE` | Info wells              |
...
| ------------ | ------------------------ | ------------------------------------------------- |
| White        | Amptron Navy             | Hero headlines, dark-section titles               |
| Mist         | Amptron Navy             | Hero lead, footer supporting copy                 |
| Navy Deep    | Signal Teal              | Primary buttons, skip-link, numbers band numerals |
| Amptron Navy | Signal Teal              | Alternate on teal badges if deep feels too dark   |
| Signal Teal  | Amptron Navy / Navy Deep | Eyebrows, ghost-button hover, outline chips       |
| Signal Teal  | White / Fog              | Text links, product “View details,” icon strokes  |
| Amptron Navy | White                    | Default body                                      |
| Slate        | White / Fog              | Supporting paragraphs                             |
| White        | Navy Deep                | Footer headings                                   |
| Amptron Navy | Soft Teal                | Icon containers (Why Amptron)                     |
| Amptron Navy | Fog                      | Card titles                                       |

**Hero badge:** Navy Deep fill, 1px Signal Teal border, white 12px uppercase label.
...
3. **Print:** specify Amptron Navy and Signal Teal as spot if budget allows; otherwise convert from the hex above. Do not pick a “close” teal from a printer’s swatch book.
4. **Photography:** let scooter photos sit on Fog or White; do not colour-wash images teal.
5. **Data viz:** Navy for the series, Teal for highlight, Slate for axes. Never red/green for series colour if it collides with semantic success/danger.
...
Hard-coded hex in new CSS is a smell unless it is a one-off hairline already in the system (e.g. existing `#12305c` admin row). Prefer tokens.
...
| ------- | ---------- | ------------------ | ---------------- |
| Display | **Outfit** | 600, 700, 800, 900 | `var(--display)` |
| Body    | **Geist**  | 400, 600, 700      | `var(--body)`    |

Loaded in `index.html` from Google Fonts. Do not substitute Inter, Poppins, Montserrat, or system-ui for brand surfaces.
...
| -------------------- | ------ | ------ | ---------------------- | -------- | -------------------- | -------------------------------------- |
| Hero h1              | Outfit | 900    | 26px → 56px            | none     | White on navy        | Line-height 1.1. Fluid `--fs-hero`     |
| Section title        | Outfit | 800    | 22px → 40px            | none     | Navy (White on dark) | Line-height 1.2. Fluid `--fs-title`    |
| Card / product title | Outfit | 800    | 17px → 24px            | none     | Navy                 | Fluid `--fs-card`                      |
| Nav links            | Outfit | 600    | 13–15px                | none     | Navy                 | Hover: Teal. Fluid `--fs-ui`           |
| Buttons              | Outfit | 700    | 13–15px                | 0.02em   | Navy Deep on Teal    | **Uppercase**. Fluid `--fs-ui`         |
| Eyebrow              | Outfit | 700    | 12–14px                | 0.06em   | Teal                 | **Uppercase** + 12×4px teal bar        |
| Stats / numbers      | Outfit | 900    | 24px → 48px            | none     | Navy on Teal band    | Fluid `--fs-stat`                      |
| Spec values          | Outfit | 700    | 15px                   | none     | Navy                 | Labels in Geist Slate                  |
| Body                 | Geist  | 400    | 15–16px                | none     | Navy                 | Line-height 1.5–1.6. Fluid `--fs-body` |
| Lead / subtitle      | Geist  | 400    | 14.5–18px              | none     | Slate or Mist        | Max ~720px measure. Fluid `--fs-lead`  |
...
| Element                     | Radius           |
| --------------------------- | ---------------- |
| Buttons, inputs, skip-link  | **8px**          |
| Icon wells, small chips     | **12px**         |
| Cards, media frames, quotes | **16px**         |
...
- Alternate White and Fog sections; one Navy hero; one Navy engineering band; one Teal stats band; Navy Deep footer. Do not stack two teal bands.
...
```
Name
Role | Amptron Manufacturing Pvt. Ltd.
+91 …
name@amptron.co.in
www.amptron.co.in
```
...
- [ ] Spelled **Amptron** (not Amtron).
- [ ] Model names are **Amptron + Volt/Storm/Cruise**.
- [ ] Logo is an official SVG, not typed letters.
- [ ] Slash is Logo Green; buttons and icons are Signal Teal.
- [ ] Headlines are Outfit; body is Geist.
- [ ] Primary CTA is teal on navy-deep type, uppercase on marketing surfaces.
...
| Need                    | Path                                     |
| ----------------------- | ---------------------------------------- |
| Navy wordmark           | `src/assets/images/logo.svg`             |
| White wordmark          | `src/assets/images/logo-light.svg`       |
| A-mark / favicon        | `public/favicon.svg`                     |
| Colour and type tokens  | `src/index.css`                          |
| Component patterns      | `src/App.css`                            |
